...
description: >
  Technical guide for creating a new CoHalo agent adapter. Use when building
...
# Creating a CoHalo Agent Adapter
...
## 2. Shared Types (`@cohalo/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@cohalo/adapter-utils` (types) or `@cohalo/adapter-utils/server-utils` (runtime helpers).
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@cohalo/adapter-utils/server-utils`)
2. **Build environment** — call `buildCoHaloEnv(agent)` then layer in `COHALO_RUN_ID`, context vars (`COHALO_TASK_ID`, `COHALO_WAKE_REASON`, `COHALO_WAKE_COMMENT_ID`, `COHALO_APPROVAL_ID`, `COHALO_APPROVAL_STATUS`, `COHALO_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
Pretty-prints stdout lines for `cohalo run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@cohalo/adapter-my-agent/server";
...
```ts
import { printMyAgentStreamEvent } from "@cohalo/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so CoHalo wipes the stale session
...
- Package name: `@cohalo/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your CoHalo work."`
...
### CoHalo Skills Injection

CoHalo ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `cohalo` API skill and the `cohalo-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.
...
1. At execution time, create a fresh tmpdir: `mkdtemp("cohalo-skills-")`
...
- `COHALO_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the CoHalo API and GitHub should not have access to arbitrary hosts.
