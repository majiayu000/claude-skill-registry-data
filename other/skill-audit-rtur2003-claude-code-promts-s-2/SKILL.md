...
With no arguments, checks `.claude/skills/` (project) and `~/.claude/skills/` (personal) if they exist. Pass explicit paths to also check a plugin's `skills/` directory. Exit code 0 = clean, 1 = findings, 2 = usage error.
...
|---|---|
| Frontmatter parses | A `SKILL.md` without valid `---`-delimited YAML at the top cannot be discovered reliably by the host |
