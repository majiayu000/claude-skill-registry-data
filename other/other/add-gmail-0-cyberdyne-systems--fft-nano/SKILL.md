---
name: add-gmail
description: Add Gmail integration to FFT_nano. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to FFT_nano. It can be configured in two modes:
...
> How do you want to use Gmail with FFT_nano?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "FFT_nano"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "FFT_nano Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep fft_nano
```
...
> - Create a Gmail label (e.g., "FFT_nano")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'FFT_nano',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/fft_nano.log | grep -i email
```
...
```bash
tail -f logs/fft_nano.log | grep -E "(email|Email)"
```
