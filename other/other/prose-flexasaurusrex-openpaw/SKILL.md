...
metadata: { "openpaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## OpenPaw Runtime Mapping

- **Task tool** in the upstream spec == OpenPaw `sessions_spawn`
- **File I/O** == OpenPaw `read`/`write`
- **Remote fetch** == OpenPaw `web_fetch` (or `exec` with curl when POST is required)
