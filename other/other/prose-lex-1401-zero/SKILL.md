...
metadata: {"zero":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Zero `sessions_spawn`
- **File I/O** == Zero `read`/`write`
- **Remote fetch** == Zero `web_fetch` (or `exec` with curl when POST is required)
