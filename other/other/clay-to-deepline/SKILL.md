...
- **2-level max interpolation**: `{{col.field}}` only works when the stored cell actually has `field` at the root. `{{col.field.nested}}` still fails. Flatten first (CLI) or go multi-level in Python.
...
- **Structured output access**: In `deepline enrich`, do not assume `deeplineagent` with `jsonSchema` stores a bare object at the cell root. Passing `{{col}}` as whole-context is reliable; if you need a specific field later, flatten it into a dedicated scalar column with `run_javascript` first. For Python SDK flows, parse the JSON and access the nested keys directly in Python.
